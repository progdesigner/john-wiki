...
## 배포 경로 이중화 발견 (2026-09-25) — 및 원인 세션 소급 확인 (2026-09-11/12)
프로덕션이 **8월 3일부터 이 모노레포가 아니라 형제 저장소 [[cwc-system]]의 `apps/elevino-*`에서
수동 배포되고 있었음**이 [[2026-09-25-엘레비노-예약취소-크레딧환불-배포장애]] 세션에서 처음
드러남 — 이 저장소의 `dev`를 그대로 배포하자 cwc-system 쪽에만 있던 기능(cwc 브리지·와인 모듈,
소장 신청 중단/취소, 모바일 관리자 메뉴 등)이 프로덕션에서 사라졌다가 3-way 머지로 복구됨. 같은
세션에서 오래된 `ServiceLoader.onModuleInit`의 잠재 버그(provider 이름이 문자열이 아니면
TypeError로 부팅 실패)가 롤백 시도 중 드러나 방어 코드로 수정. → 절차는
[[prod-rollback-source-of-truth-verify]] 스킬 참고.

**소급 확인(2026-09-26 ingest)**: 위에서 "사라졌던" 두 기능(소장 신청 중단/취소, 모바일 관리자
메뉴)의 원 구현 세션이 [[2026-09-11-elevino-admin모바일메뉴-소장신청차단취소]]로 확인됨 —
2026-09-11~12에 작업 폴더를 처음부터 `cwc-system/apps/elevino-admin`으로 잡고
`admin.elevino.io` 운영 주소에 직접 배포했으며, elevino-system(이 모노레포)의 `dev`에는 반영된
적이 없었다. 즉 09-25에 "발견"된 배포 경로 이중화는 최소 09-11부터 진행 중이었음이 이 세션으로
시점상 실증됨.
