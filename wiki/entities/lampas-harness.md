...
## 위키 아카이브 자동 저장 버그 수정 + 백필 (2026-09-26, `apps/browser` 이관 바로 다음 세션)

[[2026-09-26-브라우저탭-좌표클릭-원격직접입력-위키백필-env노출-빌드동기화]] 세션에서 발견·수정.

- **버그**: 웹의 "대화보관"이 세션 중지 후 **숨기기(hide)만** 호출하고 위키 저장(archive) 요청은
  보내지 않고 있었다 — 보관된 70건 중 **68건에 저장 잡이 아예 없었음**. [[2026-07-13-보관시-자동-기억저장]]
  이 구축한 "보관 = 아카이빙 + 장기기억 저장" 통합이, 이 특정 UI 경로(대화 목록의 "대화보관")에서는
  실제로 적용되지 않고 있었다는 뜻 — 통합 자체가 불완전했음이 두 달 넘게 지나서야 드러남.
- **수정**: 서버가 숨김 처리 직후 자동으로 위키 저장 잡을 큐에 넣도록 변경. 트랜스크립트를 못 찾으면
  이유를 세션에 기록하고, 설정 화면의 "위키 아카이브" 버튼 hover 시 그 이유가 보이게 함.
- **백필**: 운영 데몬의 archive API로 기존 68건 재처리 — **55건 큐잉, 13건은 정말로 저장할 대화가
  없음**(CLI 시작 화면뿐인 초단문 세션·codex rollout 파일 바인딩 실패)으로 확정. 잡 1건당 약 $0.7,
  총 40달러 안팎 예상.
- **`WIKI_DIR` 경로 확정**: `.env`의 `WIKI_DIR`이 `john-wiki`임을 실측 확인, `llm-wiki` 폴더는 존재하지
  않음 — [[2026-07-11-기억-요약-wiki-경로-확인]]이 남겼던 경로 불일치 우려가 이 세션에서 해소됨.
- **재시작-백필 연동**: 재시작 순간 실행 중이던 저장 잡은 데몬 규칙상 "고아" 실패 처리되므로, 재시작
  스크립트가 재시작 직후 실패 상태인 보관 세션을 찾아 archive API로 재큐잉하도록 확장(`logs/server.log`
  `[restart-archive]` 접두사로 기록). 이 세션 동안 데몬을 **두 차례** 재시작(click_at/자동저장 반영,
  이어서 원격 직접 입력 반영) — 두 번 다 이 연동으로 끊긴 잡을 자동 복구.
- → 배경: [[long-term-memory-architecture]] · 브라우저 쪽 기능(탭·`click_at`·원격 직접 입력)은
  [[lampas-browser]] 참고.

## 커밋·푸시 중 원격 히스토리 `.env` 평문 노출 발견 (같은 세션, 2026-09-26)

- "하네스 작업된거 모두 커밋하고 푸시해줘" 요청 처리 중 **푸시가 거부**돼 원격을 fetch로 확인하는
  과정에서, **9월 20일자 원격 커밋이 `.env` 파일 전체를 저장소에 커밋**해 두었음을 발견. Claude OAuth
  토큰, Anthropic·OpenAI·Google·ElevenLabs API 키, 하네스 토큰의 **실제 값**이 그 안에 들어 있었다.
- 대응: 그 커밋 위에 작업을 얹어 푸시하고, `.env` 추적을 해제하는 커밋을 추가 생성. 로컬 `.env`는
  백업 후 그대로 복원해 데몬 설정은 변경 안 됨. **추적 해제는 향후 커밋만 막을 뿐 이미 히스토리에
  남은 키는 그대로**라는 한계를 명시.
- **사용자 결정 대기 상태로 세션 종료**: (1) 노출된 키 5개+하네스 토큰 재발급, (2) 히스토리에서 해당
  커밋을 지우고 강제 푸시(다른 클론 재작업 필요) — 둘 중 어느 쪽으로 진행했는지 이 세션 소스에는 없음,
  **다음 세션에서 재확인 필요**.
- 전체 테스트 88/88 통과 후 커밋(변경 파일 80개) — 세션 브라우저(탭·`click_at`·원격 직접 입력), 위키
  자동 저장, 웹 검색 MCP, 첨부파일·네이티브 질문, Google 문서 도구, 위키 뷰어, 브랜딩 페이지, 스모크
  스크립트가 한 번에 묶여 push됨.
- → [[secrets-plaintext-exposure-pattern]]에 7번째 사례로 반영(활성 미해결 위험으로 표시). 대응
  절차 일반화: [[git-secret-discovered-in-remote-history]]

## 맥북 로컬 실행 시 빌드 산출물 미동기화 (같은 세션 마지막 요청)

- 사용자가 맥북에서 `git pull` 후 로컬 실행 시 브라우징 기능이 안 보인다고 보고. 빈 폴더에 최신
  커밋을 새로 클론해 설치·빌드를 재현한 결과 **코드는 최신이 맞고 문제는 빌드 산출물**이었다 —
  서버 `dist/`, 웹 `apps/web/dist/`, 브라우저 앱 `apps/browser/dist/`가 모두 git 제외 대상이라
  `git pull`만으로는 옛 빌드가 그대로 실행된다.
- 안내: `git pull` → `npm install` → `npm run build` → 데몬 재시작. 빌드는 **서버→브라우저 앱→웹
  순서로 이어지므로** 중간 단계 실패 시 웹 번들이 갱신 안 됨 — 이번 스크린샷이 그 증상. 또한 위
