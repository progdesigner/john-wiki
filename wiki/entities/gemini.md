...
- **이미지 생성 기본 경로** — 멀티 이미지 그리드를 직접 생성. Atlas Cloud 경유(`gpt-image-2`·
  ...
  최대 8장 입력 지원.
- **레퍼런스 시트/분석** — 레퍼런스 시트 생성 모델 선택지(Gemini 기본 / Atlas Cloud). 분석은 Grok / Gemini
  (모델 배정: 레퍼런스 분석 Grok 1차 → **Gemini 폴백**, 프로필 이미지 생성 Gemini).
...
## 관련
- [[openai]] · [[elevenlabs]] · [[higgsfield]] (외부 AI 도구군)
