...
## 상태
설계·제안 단계(2026-07-08). 실제 [[lampas-harness]]에는 아직 미구현(LLM이 tool로 능동 검색하는
경로는 여전히 없음). 이 위키 자체가 대안 구현(사서 LLM이 CLAUDE.md 워크플로로 ingest/query하며
`index.md`를 검색 진입점으로 사용).

- **혼동 주의(2026-09-26)**: `[[lampas-harness]]`에 `apps/wiki`(wiki.html, 설정>위키 버튼)가 신설돼
  이 저장소를 웹에서 열람할 수 있게 됐지만, 이건 **사람이 보는 읽기 전용 브라우저**(검색·목차·
  위키링크·역링크)이지 이 스킬이 다루는 "LLM이 tool로 능동 검색"하는 memory provider가 아니다.
  이 스킬의 5~7단계(tool 노출·주입·검증)는 여전히 미구현 상태. → [[2026-09-26-threads기능제거-llm위키탐색기-apps-wiki-이전]]
