...
- **변종 — 운영 데몬이 아니라 다른 개발 머신에서 재현됨 (2026-09-26 관찰)**: 지금까지의 함정 4는
  전부 맥미니 운영 데몬 기준이었는데, 이번엔 **사용자의 맥북**에서 `git pull` 후 로컬 실행했더니
  최근 추가된 기능(브라우징 세션 옵션)이 안 보인다고 보고된 사례다. 서버 `dist/`, 웹
  `apps/web/dist/`, 브라우저 앱 `apps/browser/dist/`가 전부 git 제외 대상이라 `git pull`만으로는
  옛 빌드가 그대로 실행되는 건 같은 원인이지만, **원인 규명 방법이 달랐다** — 빈 폴더에 최신 커밋을
  새로 클론해 설치·빌드를 재현해서 "코드는 최신이 맞다"를 먼저 증명한 뒤에야 "그럼 문제는 로컬 빌드
  산출물"이라고 좁혔다(원격 코드 자체를 의심하는 사용자 질문에 코드 진단이 아니라 빌드 재현으로
  답한 셈). 이 프로젝트는 빌드가 **서버→브라우저 앱→웹 순서로 이어지므로**, 중간 단계(예: Electron
  다운로드가 필요한 브라우저 앱 빌드)가 실패하면 이후 단계인 웹 번들이 갱신 안 돼 옛 UI가 그대로
  남는다 — 빌드 로그 끝에서 전 단계 성공 여부를 확인하는 습관이 필요하다. → [[2026-09-26-브라우저탭-좌표클릭-원격직접입력-위키백필-env노출-빌드동기화]]
