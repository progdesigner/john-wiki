...
## 관련이지만 별개 — `lampas-harness` 자체 웹 검색 도구 (2026-09-20)

위 다섯 사용처와 같은 날, **전혀 다른 저장소**(`lampas-system` 계열이 아니라 하네스 자체
`[[lampas-harness]]`)에서 같은 벤더(TypeSafe)의 키가 또 한 번 연결됐다 — 우연히 겹친 별개 사건이라
이 토픽의 "사용처" 번호에는 넣지 않고 여기 각주로만 남긴다.

- **통합 방식이 다르다**: 위 다섯 사례는 전부 `POST /v1/ai/systemone` typed 분류 게이트웨이를 거치는
  이 토픽의 정의(Jev)를 따르지만, 하네스 쪽은 TypeSafe를 **문서 관련성 평가 API로 직접 호출**해
  `github.com/superagents-lab/jev-search` 참고 구현(Jev=검색 의도 해석·결과 평가, Search1API=실제
  검색 수행)을 그대로 이식한 것 — 하네스 자체의 에이전트 웹 검색 도구(DuckDuckGo 기본, Search1API
  키 설정 시 전환)에 관련성 재정렬 단계를 얹은 구조다.
- **목적도 다르다**: 위 다섯 사례는 `lampas-system`/`dalar` 제품의 기능(채팅 의도분류·오류 triage·
  콘텐츠 게이팅)이고, 이쪽은 하네스가 에이전트(Codex/Claude 세션)에게 제공하는 **범용 검색 tool**의
  품질 개선이다.
...
- 세션: [[2026-09-20-jev-typesafe-어댑터-dalar의도분류-sportswiki게이트-구현]] ·
  ...
  [[2026-09-20-하네스터미널-jev검색연동-질문선택ui-보관세션종료-파일첨부]](별개 저장소·별개 통합방식)
