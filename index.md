...
- [[lampas-web-edit]] — lampas-system 스포츠 클립 파이프라인 "Edit"(edit.lampas.io). 클립 자막·큐 타이밍·텍스트/이미지/영상 레이어 스택·계정 종속 레이아웃 템플릿까지 담당하는 실제 편집기, 미리보기·최종렌더가 항상 같은 함수 공유. **2026-09-18**(하루 앞선 세션, 뒤늦게 09-26 ingest): 9:16 Auto Reframe(모션 무게중심 팬, 이후 수동 키프레임·Contain모드 버그 수정 3연속)·단어강조(karaoke) 자막·브랜드킷 자동적용·ASR 청크분할(긴 구간 자막 끊김 수정) 신설 → [[lampas-clip-intelligence]]. 2026-09-19~20 세션에서 0.1.17→0.1.43 20개 이상 기능을 한 세션에 연속 구현·배포(이 위키 최초 상세 노출) — 자막 여백·번역·타이포, 큐 합치기/추가, 레이어 통합, 레이아웃 템플릿 등록/적용+계정종속화+타계정 전송, presigned S3 직접업로드로 갤러리 저장 오류 근본수정
