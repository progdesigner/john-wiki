...
- [[lampas-agent]] — lampas-system 내 스포츠 클립 라벨링·업로드 맥미니 데몬(Clips·Pulse·Fixs 탭, Threads는 2026-09-26 추가 후 전면 제거됨). **[[lampas]](하네스 에이전트)와 이름만 겹치는 별개 앱** — Copy/Reels/Status/Tools 자매 앱, sports-wiki "경기" 엔티티 신설 포함. **위키 최초 노출은 2026-09-18로 재정정**(`lampas-agent-clips`+`lampas-agent-pulse` 두 앱이 이날 `apps/lampas-agent`로 병합됨과 동시에 [[omnara]] 비교분석 기반 durable-run 구현·배포 — daemon v1.0.1이 버전이력 최초 지점. 09-26 뒤늦은 ingest로 09-19 기록을 하루 정정). 09-19: Clips 유튜브 재생목록 다중선택 임포트 신설, Pulse 스케줄 수집을 4라운드에 걸쳐 Google 로그인 계정 소유자 기반으로 전환, 훅 점수를 절대 점수→텍스트 기반 상대 순위로 전환 — 이 전환은 하루 전 [[2026-09-18-lampas-clip-intelligence-brand-kit-대량구현]] 세션이 라벨링에 갓 추가한 4축 점수를 되돌린 것. **Fixs는 2026-09-21 최초 신설**(오류자동수집→Codex수정→검증→재배포, 기본 자동수정·배포 활성 하루 5회) → 2026-09-25 작업삭제(v1.0.22)→오류수집 확장(v1.0.23)→경로 그룹핑+Jev 분류(v1.0.25) 순 업그레이드. Pulse(페르소나·카피 채점) 상세는 2026-09-24 세션에서 확정
