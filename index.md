...
- [[lampas-studio]] — Lampas AI 이미지 생성 스튜디오 제품 (lampas-api + lampas-web-sdk/lampas-web-studio + lampas-web-product, sdk.lampas.io). 2026-09-26: `lampas-system` 저장소가 Lampas+Dalar+Talk 3개 라인으로 확장된 정황, 신규 앱 lampas-web-fit(fit.lampas.io) — **단 이 구조 자체는 09-21 세션(Fixs 신설 세션)에도 이미 존재했음이 09-26 ingest로 확인**, "최초 노출"은 위키 관찰 시점일 뿐. 2026-09-24: 동영상 생성에 Seedance 2.0 Mini 추가+노드 캔버스 좌우 버튼 UX 개선 배포, Dalar SoT 관계를 `pnpm sync:studio` 실행으로 실증. 2026-09-21: 이미지 Edit 모델 2종(Sunburst·Flare)+Qwen Image 3.0 Pro Edit 추가, 영상 WAN 2.7→3.0(이미지는 2.7 Pro 유지)+멀티이미지 레퍼런스, `models.lampas.io` 508개 모델 동기화, "생성 중단 오표시" 반복 버그 근본수정
- [[dalar]] — `lampas-system` 모노레포 내 제품 라인, 이제 7앱 확인(`dalar-web-root/www/cs/app/admin`+`dalar-api`+**`dalar-web-first`**). Node Studio SoT가 여기 위치해 lampas-web-studio로 동기화. 2026-09-24: SoT 관계가 문서 인용을 넘어 실제 sync 실행·드리프트 0으로 확인됨. 2026-09-20(2026-09-26 뒤늦게 ingest): 실사용 소비자 제품 [[dalar-web-first]]가 대규모 개발·배포됨이 확인 — `AGENTS.md` 인용 스텁을 넘어 실제 코드 경로·운영 도메인까지 확인된 첫 사례
