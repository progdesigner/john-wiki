...
- [[dalar-web-first]] — Dalar 소속 "First" AI 돌잔치 인터뷰 영상 서비스(`first.dalar.ai`/`admin.first.dalar.ai`). **origin은 2026-09-13/14 세션**(Gmail 로그인+가입크레딧, 5만 크레딧 고정 결제, `admin.first.dalar.ai` 인증서 ACM 권한 부재로 `/admin` 경로 임시 배포, 동시결제 8건 1회 차감 실증, 실사진 36초 영상 완제 검증까지 최초 구현·배포). 이후 4·6·8·12장면 선택+가격체계, Jev 재시도판단, 장면 레퍼런스 고정, 가상 아기 샘플 4편, 초대코드 상호쿠폰, Pay 분리는 2026-09-20~24 후속 세션에서 확장
...
- [[lampas-web-scenario]] — lampas-system 내 컷(샷)별 이미지→영상 생성 시나리오/스토리보드 앱. 시나리오 전용 백엔드 없이 제네릭 [[atlas-cloud]] `generateVideo` 게이트웨이 패스스루. 2026-09-13: 이미지+텍스트+오디오 아바타 모델 4종 추가, 길이 10초 고정→5~30초 슬라이더+자동추정, 카메라 고정 프롬프트 자동삽입
- [[lampas-web-trends]] — lampas-system 내 트렌드 키워드 수집·분석 앱(trends.lampas.io) + lampas-trends-collector(Python 수집기). **2026-09-15 세션에서 이 위키에 처음 상세 노출**(정정 — 이전엔 09-19로 기록): `GET /v1/trends/search` 실시간 검색 엔드포인트 최초 구현(분야+세부 키워드, 2분 캐시·IP 분당20회 제한). 2026-09-19 후속 고도화: 토픽 단위 재구축, 기사 제목·인기글 핵심 키워드 3개 유추(Atlas Cloud 경유 gemini-3.5-flash 배치, 24h 캐시)+2건 이상 반복돼야 순위 채택, X·Threads 공식 API 소스 추가, 운영 배포까지 완료(collector 0.1.7·API 0.1.99). Threads 토큰 만료(9/10)·X 토큰 미설정으로 소셜 소스 13개씩 불통, 코드는 미커밋
