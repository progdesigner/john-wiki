...
- [[2026-09-19-lampas-trends-고도화]] — `Tool: claude` 세션(2026-09-26 뒤늦게 ingest). 트렌드 수집기를 토픽 단위로 재구축 + 기사 제목·인기글마다 핵심 키워드 3개 유추(Atlas Cloud 경유 gemini-3.5-flash, 키 없으면 규칙 폴백) + X·Threads 공식 API 소스 추가·구현·검증 후 운영 배포까지 완료. 배포 중 40개 배치 타임아웃/응답잘림으로 모델 유추 전량 실패를 배치 20개·타임아웃 90초·백그라운드 캐시 워밍으로 근본수정. `[[lampas-web-trends]]`·`lampas-trends-collector`가 이 위키에 처음 상세 노출. Threads 토큰 만료·X 토큰 미설정은 운영 조치 필요로 남고 코드는 미커밋 상태로 종료
...
- [[lampas-web-trends]] — lampas-system 내 트렌드 키워드 수집·분석 앱(trends.lampas.io) + lampas-trends-collector(Python 수집기). 2026-09-19 세션에서 이 위키에 처음 상세 노출: 토픽 단위 재구축, 기사 제목·인기글 핵심 키워드 3개 유추(Atlas Cloud 경유 gemini-3.5-flash 배치, 24h 캐시)+2건 이상 반복돼야 순위 채택, X·Threads 공식 API 소스 추가, 운영 배포까지 완료(collector 0.1.7·API 0.1.99). Threads 토큰 만료(9/10)·X 토큰 미설정으로 소셜 소스 13개씩 불통, 코드는 미커밋
