...
- [[john-wiki]] — 이 위키. 저장 진입점: 상단 "기억에 보관" 버튼(2026-07-16~ 구 "기억에 저장"에서 개명, 보관 API로 통합)·대화목록 롱프레스 보관·야간 자동 ingest — 앞 둘은 이제 동일 동작. 능동 조회(검색 tool, 방법 C)는 미완. **2026-09-26**: [[lampas-harness]]에 이 저장소를 사람이 위키피디아처럼 브라우징하는 읽기전용 `apps/wiki`(wiki.html, 검색·목차·위키링크·역링크, 설정>위키 버튼) 신설·검증완료(방법 D, LLM tool과 별개) — 반영은 서버 재시작 대기 중이었음(다음 확인 필요). 계기는 [[lampas-agent]]에 요청한 "위키기반 Threads 글 생성" 기능이 위키 오인(sports-wiki↔john-wiki) 끝에 정정됐다가 전면 취소된 것.
