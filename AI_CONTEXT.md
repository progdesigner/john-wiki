...
- [[lampas-studio]] — 이미지 생성 스튜디오 제품(로컬 저장소명 `lampas-system`). "Edit" 템플릿 에디터(이미지 슬롯 `fingerprint`(로컬)/`url`(공개 CDN) 이원 구조) → [[template-image-slot-fingerprint-vs-url]]. **2026-09-26 AGENTS.md 스냅샷**: 저장소가 Lampas+**Dalar**(신규 노출, [[dalar]])+**Talk**(구 dbs/talk-system 통합 정황, [[toktalk]] "별개 코드베이스" 기록과 모순·미확인) 3라인으로 확장, DB가 MySQL로 명시(2026-07 확인된 PostgreSQL과 모순, 미해결). 신규 앱 **lampas-web-fit**(음악 박자 동기 운동 앱) 구현·`fit.lampas.io` 배포 완료 → [[2026-09-25-lampas-web-fit-구축-배포]].
