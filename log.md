...
## [2026-08-24] lint | 위키 전체 건강 점검 (6) — 무결성 재검증, 결함 0
- **범위**: 세션 90·엔티티 25·토픽 24·스킬 39 전수 + index.md·AI_CONTEXT.md·프론트매터·raw↔세션 커버리지.
  마지막 lint(2026-08-10 #5) 이후 신규 ingest **0건**(log.md·git 이력 확인) — 드리프트 원인 없음.
- **기계 점검 (스크립트, 모두 클린)**:
  ...
  - 프론트매터: 전 파일 유형별 템플릿 준수(sessions/entities/topics `tags/created/updated` 완비, skills는
    SKILL.md 형식이라 `updated` 없음이 정상). 결함 0.
  - raw↔세션 커버리지: raw 90 = 세션 90, 모든 소스 ingest 완료. 이름 불일치 2건은 정상 리네임 케이스
    ...
    미해소 출처 링크 0.
- **고아 페이지**: 비-index 인바운드 없는 페이지 0(08-10 #5의 저가치 고아 2건도 이후 변동 없음, 신규 소스 부재).
...
- 결과: 깨진 링크 0·index 불일치 0·프론트매터 결함 0·raw 커버리지 결함 0·신규 모순 0·고아 변동 0.
  **수정 결함 없음**(억지 편집 지양). 신규/편집 페이지 없음 — lint #6은 무결성 재확인 기록만.
